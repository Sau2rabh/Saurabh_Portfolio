...
const GlowCard: React.FC<GlowCardProps> = ({
  ...
  const inlineStyles: React.CSSProperties & Record<string, string | number> = {
    ...
    '--backup-border': 'hsl(220 15% 18% / 0.6)',
    '--size': '280',
    '--outer': '1',
    ...
    backgroundImage: `radial-gradient(
      ...
      calc(var(--x, 0) * 1px)
      calc(var(--y, 0) * 1px),
      hsl(var(--hue, 190) calc(var(--saturation, 80) * 1%) calc(var(--lightness, 70) * 1%) / var(--bg-spot-opacity, 0.08)), transparent
    )`,
  ...
  return (
    <>
      ...
      <div
        ...
        `}
      >
        {/* Inner glow layer required by the CSS ::before/::after trick */}
        <div data-glow />
        {children}
      </div>
    </>
